...
public class TellerTest extends TestCase
{
	...
	public void testOneMarketBill(){
		...
		if(!cashier.debts.isEmpty()){
			cashier.debts = new ArrayList<CashierAgent.Debt>();
		}
	...
	public void testNormalSingleBill(){
		...
		if(!cashier.marketBills.isEmpty()){
			cashier.marketBills = new ArrayList<CashierAgent.MarketBill>();
		}
		...
		assertTrue("Cashier's first debt should be for waiter and cust and steak.",(cashier.debts.get(0).w==waiter)&&
				(cashier.debts.get(0).c == customer) && (cashier.debts.get(0).s==CashierAgent.DebtState.uncomputed));
		...
		assertTrue("Debt should be in the unpaid state.",cashier.debts.get(0).s==CashierAgent.DebtState.unpaid);
		...
		assertTrue("Debt should be in the needsChange state.",cashier.debts.get(0).s==CashierAgent.DebtState.needsChange);
		...
		assertTrue("Cashier should be finished with the debt.",cashier.debts.get(0).s==CashierAgent.DebtState.paid);
	...
	public void testCantPayOneBill(){
		...
		assertTrue("Debt should be in the cantPay state.",cashier.debts.get(0).s==CashierAgent.DebtState.cantPay);
		assertTrue("Cashier state should have a new permit",cashier.stateChange.availablePermits()==3);
		...
		assertTrue("Cashier should debt in delinquent state.",cashier.debts.get(0).s==CashierAgent.DebtState.delinquent);
	...
	public void testTwoCustBillOneMarketBill(){
		//set up for this scenario
				...
				assertTrue("Debt1 should be in the unpaid state.",cashier.debts.get(0).s==CashierAgent.DebtState.unpaid);
				...
				assertTrue("Debt1 should be in the needsChange state.",cashier.debts.get(0).s==CashierAgent.DebtState.needsChange);
				assertTrue("Cashier state should have a new permit",cashier.stateChange.availablePermits()==5);
				...
				assertTrue("Debt2 should be in the unpaid state.",cashier.debts.get(1).s==CashierAgent.DebtState.unpaid);
				...
				assertTrue("Debt2 should be in the cantPay state.",cashier.debts.get(1).s==CashierAgent.DebtState.cantPay);
				...
				assertTrue("Cashier should debt in delinquent state.",cashier.debts.get(1).s==CashierAgent.DebtState.delinquent);
